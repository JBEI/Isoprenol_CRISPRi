...
"""
def EFlux2(model, Transcriptomics, epsilon = 1e-6, fba_only = False):
    ...
    for rxn in eflux2_model.reactions:
        if 'EX_' not in str(rxn.id):
            if rxn.gene_reaction_rule:
                ...
                n_inf_bounds += 1
    print(f'There were {n_bounds_updated} bounds updated and {n_inf_bounds} bounds set to +/- inf.')
    ...
    if fba_only:
        EFlux2_solution = None
        print('skipping L2 minimization')

    else:
        print('running L2 minimization')
    ...
    # constrain the biomass to the optimal value
        for r in eflux2_model.reactions:
            if r.objective_coefficient:
                r.lower_bound = fba_solution.objective_value-epsilon
                r.upper_bound = fba_solution.objective_value+epsilon
    ...
    #     display(eflux2_model.medium)
        ...
        eflux2_model.solver.verbosity = 1
    ...
    return EFlux2_solution, fba_solution

def boundedFBA(model, Transcriptomics, epsilon = 1e-6):
    print(f'using epsilon = {epsilon}')
    # copy model and set tolerance
    bfba_model = model.copy()
    bfba_model.tolerance = 1e-9
    
    # set the flux bounds for each reaction using the transcriptomics data    
    update_flag = False
    n_bounds_updated = 0
    for rxn in bfba_model.reactions:
        if 'EX_' not in str(rxn.id):
            if rxn.gene_reaction_rule:
                if rxn.lower_bound < 0.0:
                    rxn.lower_bound = -transcript_value_for_rxn(model, Transcriptomics, rxn)
                    ...
                    if update_flag == False:
                        update_flag = True
                        print(f'Bound updated: {rxn.id}, lower bound = {rxn.lower_bound}')
                else:
                    pass
                if rxn.upper_bound > 0.0:
                    rxn.upper_bound = transcript_value_for_rxn(model, Transcriptomics, rxn)
                ...
                if rxn.upper_bound >= 1000:
                    rxn.upper_bound = np.Inf 
    print(f'There were {n_bounds_updated} bounds updated')
    # solve FBA problem with transcriptomic bounds
    fba_solution = bfba_model.optimize()
    print('FBA status', fba_solution.status)
    print('FBA solution', fba_solution.objective_value)
    return fba_solution
#############################################
